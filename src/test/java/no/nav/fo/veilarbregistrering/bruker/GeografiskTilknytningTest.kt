...
class GeografiskTilknytningTest {
    ...
    fun `exception skal kastes hvis geografiskTilknytning er ukjent`() {
        val illegalArgumentException = assertThrows<IllegalArgumentException> {
            GeografiskTilknytning("12").value()
        }
    ...
    fun `bydel bjerke i oslo er ikke by med bydel`() {
        val bjerke = GeografiskTilknytning(GeografiskTilknytning.BydelOslo.Bjerke.kode())
        assertThat(bjerke.byMedBydeler()).isFalse
    }

    @Test
    fun `oslo er by med bydeler`() {
        val oslo = GeografiskTilknytning(GeografiskTilknytning.ByMedBydeler.Oslo.kode())
        assertThat(oslo.byMedBydeler()).isTrue
    }

    @Test
    fun `stavanger er by med bydeler`() {
        val oslo = GeografiskTilknytning(GeografiskTilknytning.ByMedBydeler.Stavanger.kode())
        assertThat(oslo.byMedBydeler()).isTrue
    }

    @Test
    fun `bergen er by med bydeler`() {
        val oslo = GeografiskTilknytning(GeografiskTilknytning.ByMedBydeler.Bergen.kode())
        assertThat(oslo.byMedBydeler()).isTrue
    }

    @Test
    fun `trondheim er by med bydeler`() {
        val oslo = GeografiskTilknytning(GeografiskTilknytning.ByMedBydeler.Trondheim.kode())
        assertThat(oslo.byMedBydeler()).isTrue
    }
}
