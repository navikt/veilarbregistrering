...
class PeriodeTest {
    ...
    fun `fraDatoAs yyyyMMdd skal skrive ut fradato pa formatet yyyyMMdd`() {
        val periode = Periode.of(LocalDate.of(2020, 1, 12), LocalDate.of(2020, 2, 20))
    ...
    fun `tildato er innenfor forespurt periode`() {
        ...
        val periodeMedTildato = Periode.of(FØRSTE_JANUAR, FØRSTE_APRIL)
        val forespurtPeriode = Periode.of(FØRSTE_FEBRUAR, FØRSTE_MAI)
        assertThat(periodeMedTildato.overlapperMed(forespurtPeriode)).isTrue
    }

    @Test
    fun `periode er avs`() {
        val periodeMedTildato = Periode.of(FØRSTE_JANUAR, LocalDate.of(2020, 1, 31))
        val forespurtPeriode = Periode.of(FØRSTE_FEBRUAR, FØRSTE_MAI)
    ...
    @Test
    fun `periode ar avs`() {
        val periodeMedTildato = Periode.of(LocalDate.of(2020, 6, 1), LocalDate.of(2020, 8, 31))
        val forespurtPeriode = Periode.of(FØRSTE_FEBRUAR, FØRSTE_MAI)
        assertThat(periodeMedTildato.overlapperMed(forespurtPeriode)).isFalse
    }
    ...
    fun `forespurtPeriode med åpen tildato`() {
        val periodeMedTildato = Periode.of(FØRSTE_JANUAR, FØRSTE_APRIL)
        val forespurtPeriode = Periode.of(FØRSTE_FEBRUAR, null)
        assertThat(periodeMedTildato.overlapperMed(forespurtPeriode)).isTrue
    }

    @Test
    fun `periode med åpen tildato`() {
        val periodeMedTildato = Periode.of(FØRSTE_JANUAR, null)
        val forespurtPeriode = Periode.of(FØRSTE_FEBRUAR, FØRSTE_APRIL)
        assertThat(periodeMedTildato.overlapperMed(forespurtPeriode)).isTrue
    }
