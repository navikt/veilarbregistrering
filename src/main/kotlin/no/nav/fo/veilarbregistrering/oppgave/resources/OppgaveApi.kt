...
interface OppgaveApi {
    @Operation(summary = "Oppretter 'kontakt bruker'-oppgave på vegne av bruker.")
    @ApiResponses(
        ApiResponse(responseCode = "200", description = "Oppgave opprettet OK"), ApiResponse(
            responseCode = "403", description = "Duplikat - fant tilsvarende oppgave " +
                    "som ble opprettet innenfor de siste 2 arbeidsdager"
        ), ApiResponse(responseCode = "500", description = "Ukjent teknisk feil")
    )
    fun opprettOppgave(oppgaveDto: OppgaveDto?): OppgaveDto
}